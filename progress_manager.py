...
class MainWindow(QMainWindow):
        ...
        if os.path.exists(self.data_file):
            ...
            return {
                "山田 太郎 (1年)": {
                    "research": "自然言語処理の研究",
                    "tasks": [
                        {"name": "論文調査", "due": "2025-07-01", "state": "完了"},
                        {"name": "実験データ収集", "due": "2025-07-10", "state": "進行中"}
                    ],
                    "memo": "今週は進捗良好。"
                },
                "鈴木 花子 (2年)": {
                    "research": "機械学習の応用",
                    "tasks": [
                        {"name": "モデル実装", "due": "2025-06-30", "state": "進行中"},
                        {"name": "結果分析", "due": "2025-07-05", "state": "未着手"}
                    ],
                    "memo": "タスクの優先順位付けが必要。"
                }
            }
    ...
    def save_student_data(self):
        ...
        for row in range(self.task_table.rowCount()):
            ...
            if name_item and date_widget and state_widget:
        ...
        self.save_all_data_to_file()  # ここで丸ごと保存
